...
int randInt(int lowest, int highest)
{
    ...
    //returns a random integer value bounded in the grid
}
...
{
    int rnew = r;//saves old position to the new one
    int cnew = c;
    switch (dir)
    {
      case NORTH:  if (r <= 1)        return false; else rnew--; break;//can't go beyond top row
      case EAST:   if (c >= a.cols()) return false; else cnew++; break;//can't go beyond lowest column
      case SOUTH:  if (r >= a.rows()) return false; else rnew++; break;//can't go beyond rightmost column
      case WEST:   if (c <= 1)        return false; else cnew--; break;//can't go beyond leftmost column
    }
    r = rnew;
    c = cnew;
    return true;
